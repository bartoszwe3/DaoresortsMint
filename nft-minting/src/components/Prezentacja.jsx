...
const SECTIONS = [
    ...
    { id: "s10", label: "Gdzie idą Twoje wpłaty" },
    { id: "s11", label: "FAQ" },
    { id: "s12", label: "Dołącz" },
];
    ...
    <Section id="s2" className="py-12 md:py-32 px-6">
        <div className="max-w-5xl mx-auto">
            <div className="text-center mb-16">
                ...
                <h2 className="font-playfair text-3xl md:text-5xl text-[#F5F0E8] mb-4">Ile naprawdę kosztują Twoje wakacje?</h2>
                <p className="text-[#8A9E8A] text-base md:text-lg max-w-2xl mx-auto">Hotel premium z jacuzzi: 5 600 PLN za tydzień, czyli 11 200 PLN za 14 nocy rocznie przez 20 lat. Policz sam.</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-12">
                <CounterCard value={224000} suffix=" PLN" label="Koszty hotelowe" sublabel="11 200 PLN/rok × 20 lat" />
                <CounterCard value={146000} prefix="+" suffix=" PLN" label="Inflacja hotelowa" sublabel="5% wzrostu cen rocznie" />
                <CounterCard value={370000} suffix=" PLN" label="Łączny wydatek" sublabel="Twoje 20 lat wakacji premium" accent />
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <ul className="space-y-3">
                            {["Zero własności", "Zero wpływu na jakość", "Zero kontroli nad ceną", "Zdjęcia na Instagramie"].map((item, i) => (
    ...
    <Section id="s3" className="py-12 md:py-32 px-6">
        <div className="max-w-5xl mx-auto">
            <div className="text-center mb-16">
                ...
                <h2 className="font-playfair text-3xl md:text-5xl text-[#F5F0E8] mb-4">Płacisz raz. Korzystasz <span className="text-[#C9A84C]">dożywotnio.</span></h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                    { icon: <Key size={28} className="text-[#C9A84C]" />, title: "Płacisz raz", desc: "19 990 PLN jednorazowo. Żadnych rocznych opłat członkowskich. Żadnych ukrytych kosztów wejścia." },
                    { icon: <Home size={28} className="text-[#C9A84C]" />, title: "Korzystasz po kosztach", desc: "~692 PLN za tydzień zamiast 5 600 PLN w hotelu. Płacisz tylko realne koszty mediów i sprzątania." },
                    { icon: <Shield size={28} className="text-[#C9A84C]" />, title: "Jesteś właścicielem", desc: "Karta Członkowska to Twój prawny dokument prawa do korzystania. Możesz ją sprzedać, przekazać rodzinie lub zachować na zawsze." }
                ].map((card, i) => (
    ...
    <Section id="s8" className="py-12 md:py-32 px-6">
        <div className="max-w-4xl mx-auto">
            <div className="text-center mb-16">
                ...
                <h2 className="font-playfair text-3xl md:text-5xl text-[#F5F0E8]">Człowiek za projektem</h2>
            </div>
            <div className="flex flex-col md:flex-row items-center md:items-start gap-10 bg-[#1C2614]/30 border border-[#2D5A3D]/30 rounded-2xl p-8 md:p-10 text-left">
                ...
                <div className="flex-1">
                    ...
                    <div className="pt-8 border-t border-[#2D5A3D]/30 min-h-[160px]">
                        <p className="text-[#F5F0E8] text-sm mb-4 font-medium italic">Masz pytania? Zostaw numer - oddzwonię osobiście.</p>
                        {leadStatus === 'success' ? (
                            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-[#C9A84C]/10 border border-[#C9A84C]/30 p-4 rounded-xl text-[#C9A84C] text-sm font-medium">Oddzwonię wkrótce. - Bartosz</motion.div>
                        ) : (
                            <div className="flex flex-col sm:flex-row gap-3">
                                <input type="text" placeholder="Imię" required className="bg-[#0E1208] border border-[#2D5A3D] rounded-none px-4 py-2.5 text-sm text-[#F5F0E8] focus:outline-none focus:border-[#C9A84C] transition-colors flex-1" value={leadForm.imie} onChange={(e) => setLeadForm({...leadForm, imie: e.target.value})} />
                                <input type="tel" placeholder="Numer telefonu" required className="bg-[#0E1208] border border-[#2D5A3D] rounded-none px-4 py-2.5 text-sm text-[#F5F0E8] focus:outline-none focus:border-[#C9A84C] transition-colors flex-1" value={leadForm.telefon} onChange={(e) => setLeadForm({...leadForm, telefon: e.target.value})} />
                                <button type="button" onClick={handleLeadSubmit} disabled={leadStatus === 'submitting'} className="bg-[#C9A84C] hover:bg-[#b09342] text-[#0E1208] font-bold px-6 py-2.5 rounded-none transition-all shadow-btn-primary flex items-center justify-center gap-2 text-sm whitespace-nowrap disabled:opacity-50">
                                    {leadStatus === 'submitting' ? 'Wysyłanie...' : 'Zadzwoń do mnie →'}
                                </button>
                            </div>
                        )}
                        {leadStatus === 'error' && <p className="text-red-400 text-xs mt-2">Coś poszło nie tak. Spróbuj ponownie lub napisz maila.</p>}
                    </div>
                </div>
            </div>
        </div>
    ...
    <Section id="s4" className="py-20 md:py-32 px-6">
        <div className="max-w-4xl mx-auto">
            <div className="text-center mb-16">
                ...
                <h2 className="font-playfair text-3xl md:text-5xl text-[#F5F0E8]">Liczby mówią same za siebie.</h2>
            </div>
            ...
            <div className="bg-[#C9A84C]/10 border border-[#C9A84C]/20 rounded-xl p-6 text-center">
                <p className="text-[#F5F0E8] text-base md:text-lg">Członkostwo zwraca się po <span className="text-[#C9A84C] font-bold">1.3 sezonie</span>. Reszta to czyste oszczędności.</p>
            </div>
        </div>
    </Section>
);
    ...
    <Section id="s5" className="py-20 md:py-32 px-6">
        <div className="max-w-5xl mx-auto text-center">
            <div className="mb-16">
                ...
                <h2 className="font-playfair text-3xl md:text-5xl text-[#F5F0E8]">Silna, Lubuskie</h2>
                <p className="text-[#8A9E8A] text-base mt-4 max-w-2xl mx-auto">Działka 2 530 m² w lesie przy jeziorze Pszczewskim. 6 domków premium z prywatnym jacuzzi. 7. budynek operacyjny z salą kinową, pokojem gier i pralnią.</p>
            </div>
            ...
            <blockquote className="max-w-2xl mx-auto mb-14">
                <p className="font-playfair text-2xl md:text-3xl text-[#F5F0E8] italic leading-relaxed">„6 domków. 150 rodzin. Las. Jacuzzi. <span className="text-[#C9A84C]">Twoje.</span>”</p>
            </blockquote>
    ...
    <Section id="s6" className="py-20 md:py-32 px-6">
        <div className="max-w-4xl mx-auto text-center">
            <div className="mb-16">
                ...
                <h2 className="font-playfair text-3xl md:text-5xl text-[#F5F0E8]">4 kroki do Twoich wakacji</h2>
            </div>
            <div className="space-y-0 text-left">
                {[
                    { step: "01", icon: <Users size={24} />, title: "Odbierz darmowy Paszport", desc: "Rejestracja emailem. Zero skomplikowanych rejestracji — logujesz się przez link wysłany na Twój email." },
                    { step: "02", icon: <Key size={24} />, title: "Zarezerwuj członkostwo", desc: "2000 PLN wpisowego. Zabezpieczasz swoje miejsce w pierwszym etapie. Kolejne wpłaty wraz z postępem budowy." },
                    { step: "03", icon: <Calendar size={24} />, title: "Rezerwuj domek", desc: "Przez aplikację DAOResorts. 14 nocy rocznie w dowolnym dostępnym terminie. Płacisz tylko koszty operacyjne." },
                    { step: "04", icon: <Vote size={24} />, title: "Głosuj i współdecyduj", desc: "Każdy członek ma 1 głos. Decydujesz o budżecie, modernizacjach i zasadach. To Twój resort." }
                ].map((item, i) => (
            ...
            <p className="text-[#8A9E8A] text-sm mt-6 border-t border-[#2D5A3D]/20 pt-8 max-w-lg mx-auto">Płacisz zwykłym przelewem bankowym. My zadbaliśmy o resztę.</p>
        </div>
    </Section>
);
    ...
    <Section id="s7" className="py-20 md:py-32 px-6">
        <div className="max-w-5xl mx-auto text-center">
            <div className="mb-16">
                ...
                <h2 className="font-playfair text-3xl md:text-5xl text-[#F5F0E8]">Transparentność to nie slogan.</h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
                <div className="bg-[#1C2614]/40 border border-[#2D5A3D]/30 rounded-2xl p-8">
                    ...
                    <ul className="space-y-4">
                        {["Działka kupiona - Silna, Lubuskie", "Spółka zarejestrowana - KRS 0001188626", "Architekci podpisani - projekt w toku", "Aplikacja działa - daoresorts.club", "Umowa w publicznym rejestrze cyfrowym — każda złotówka widoczna"].map((fact, i) => (
                ...
                <div className="bg-[#1C2614]/40 border border-[#2D5A3D]/30 rounded-2xl p-8">
                    ...
                    <ul className="space-y-4">
                        {["100% zwrot jeśli budowa nie dojdzie do skutku", "Budowa startuje po potwierdzeniu 75 miejsc członkowskich (50%)", "Każda złotówka widoczna w panelu społeczności", "Karta Członkowska zbywalna — możesz sprzedać innemu kupującemu"].map((item, i) => (
                            <li key={i} className="flex items-start gap-3 text-sm text-[#8A9E8A]">
...
const S9 = () => {
    const stages = [
        ...
        { label: "75 potwierdzonych członków (50%)", status: "pending", desc: "Start budowy resortu" },
        ...
        <Section id="s9" className="py-20 md:py-32 px-6">
            <div className="max-w-4xl mx-auto text-center">
                <div className="mb-16">
                    ...
                    <h2 className="font-playfair text-3xl md:text-5xl text-[#F5F0E8]">Gdzie jesteśmy i co dalej</h2>
                </div>
...
const S10 = () => (
    <Section id="s10" className="py-20 md:py-32 px-6">
        <div className="max-w-4xl mx-auto text-center">
            <div className="mb-16">
                <p className="text-[#C9A84C] text-xs uppercase tracking-[0.25em] mb-4">Gdzie idą Twoje wpłaty</p>
                <h2 className="font-playfair text-3xl md:text-5xl text-[#F5F0E8]">Gdzie idą Twoje wpłaty</h2>
            </div>
            <div className="space-y-6 mb-14 text-left">
                {[
                    ...
                    { pct: 10, amount: "~1 999 PLN", label: "Fundusz remontowy wspólnoty", desc: "Twoje pieniądze na przyszłość resortu", color: "bg-green-500" },
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-left">
                <div className="bg-[#1C2614]/40 border border-[#2D5A3D]/30 rounded-2xl p-6">
                    <p className="text-[#8A9E8A] text-xs uppercase tracking-widest mb-3">Profil Gościa (bezpłatny)</p>
                ...
                <div className="bg-[#C9A84C]/10 border border-[#C9A84C]/30 rounded-xl p-6">
                    <p className="text-[#C9A84C] text-xs uppercase tracking-widest mb-3">Karta Członkowska</p>
                    ...
                    <p className="text-[#8A9E8A] text-xs">Zaczynasz od 2000 PLN wpisowego (zwrotnego). Dożywotnie prawo do 14 nocy rocznie + głos w społeczności.</p>
                </div>
            </div>
        </div>
    </Section>
...
const S11 = () => {
    ...
    const faqs = [
        { q: `„Skąd mam wiedzieć że to nie oszustwo?”`, a: "Rozumiemy sceptycyzm. Dlatego: spółka z KRS, działka w księdze wieczystej, 100% zwrot jeśli nie budujemy. Sprawdź numer KRS 0001188626 w rejestrze KRS online - to publiczna informacja." },
        { q: `„Co jeśli spółka zbankrutuje?”`, a: "Karta Członkowska jest Twoja — zapisana w publicznym rejestrze cyfrowym. Spółka zarejestrowana w KRS (numer 0001188626), działka w księdze wieczystej, dane publiczne." },
        { q: `„Nie znam się na nowoczesnych technologiach”`, a: "Płacisz zwykłym przelewem. Logujesz się emailem. Otrzymujesz Kartę Członkowską — prawny dokument potwierdzający Twoje miejsce w resorcie." },
        { q: `„14 nocy to mało”`, a: `14 nocy × 6 osób = 84 osobodoby rocznie. Przy cenie hotelu 800 PLN/noc to 67 200 PLN wartości. Ty płacisz ~692 PLN za tydzień. To nie „mało” - to matematyka.` },
        { q: `„A co jeśli chcę sprzedać?”`, a: "Karta Członkowska jest zbywalna. Możesz sprzedać swoje miejsce innemu kupującemu w dowolnym momencie. 10% wartości trafia do funduszu wspólnoty." }
    ];
    return (
        <Section id="s11" className="py-20 md:py-32 px-6">
            <div className="max-w-3xl mx-auto text-center">
                <div className="mb-16">
                    ...
                    <h2 className="font-playfair text-3xl md:text-5xl text-[#F5F0E8]">Odpowiedzi zanim zapytasz</h2>
                </div>
    ...
    return (
        <Section id="s12" className="py-20 md:py-32 px-6">
            <div className="max-w-2xl mx-auto text-center">
                <h2 className="font-playfair text-3xl md:text-5xl text-[#F5F0E8] mb-4">Zostało <span className="text-[#C9A84C]">{150 - activeMembers}</span> z 150 miejsc założycielskich.</h2>
                <p className="text-[#8A9E8A] text-base mb-8">Cena w Etapie 2 będzie wyższa. Founder price tylko teraz.</p>
                ...
                <p className="text-[#8A9E8A] text-xs mb-10">{activeMembers} / 150 zajętych</p>
                {onConnect && (
                    <div className="flex flex-col items-center gap-2 w-full px-4 md:px-0 mb-8 max-w-sm mx-auto">
                        <button onClick={() => onConnect(false)} className="px-6 py-4 w-full bg-[#C9A84C] hover:bg-[#b09342] text-[#0E1208] font-sans font-bold transition-all shadow-btn-primary flex items-center justify-center gap-2">Dołącz za darmo</button>
                        <button onClick={() => onConnect(true)} className="text-[#8A9E8A] text-xs text-center w-full mt-2 hover:text-[#C9A84C] transition-colors">Masz już konto? Zaloguj się</button>
                    </div>
                )}
                <p className="text-[#8A9E8A] text-xs">Masz pytania? <a href="mailto:bartosz@daoresorts.club" className="text-[#C9A84C] hover:underline">bartosz@daoresorts.club</a></p>
            </div>
        </Section>
    );
};
...
const Footer = () => (
    <footer className="py-12 border-t border-[#2D5A3D]/20 px-6 text-center">
        <div className="max-w-4xl mx-auto">
            ...
            <p className="text-[#8A9E8A] text-sm mb-8">Luksus na własność w sercu polskiej natury.</p>
            <div className="text-[#8A9E8A]/50 text-[10px] uppercase tracking-widest">© 2024 DAOResorts. Wszelkie prawa zastrzeżone.</div>
        </div>
    </footer>
);
...
export default function Prezentacja({ onConnect }) {
    ...
    return (
        <div className="bg-[#0E1208] min-h-screen text-[#F5F0E8] font-sans selection:bg-[#C9A84C]/30 relative">
            ...
            <S1 onConnect={onConnect} />
            <S2 />
            <S3 />
            <S8 leadForm={leadForm} setLeadForm={setLeadForm} leadStatus={leadStatus} handleLeadSubmit={handleLeadSubmit} />
            <S4 />
            <MemberCarousel />
            <S5 />
            <S6 />
